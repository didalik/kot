    ...
    close: ob.stream({
      ...
      onmessage: e => {
        if (vm.d.account && e.bids.length > 0 && !vm.d.granted) { // reset and start the demo
          ...
          configuration.promise = Promise.resolve({ jobs: [ 'reset_testnet' ], top: true })
          put_agent(null, null, null, process.env.JOBAGENT_PK, 'hx', 'GD5J36GTTAOV3ZD3KLLEEY5WES5VHRWMUTHN3YYTOLA2YR3P3KPGXGAQ')
        }
        if (vm.d.account && e.bids.length > 0 && vm.d.granted) {  // wait for next demo request
          vm.d.granted = false
        }
      }
    }),
